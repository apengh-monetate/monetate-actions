...
this.count()},count:function e(){var t=this,s=this.$.days[0],o=this.$.days[1],i=this.$.hours[0],n=this.$.hours[1],u=this.$.minutes[0],a=this.$.minutes[1],c=this.$.seconds[0],l=this.$.seconds[1];this.countdown_interval=setInterval(function(){0<t.total_seconds?(--t.values.seconds,0<=t.values.minutes&&t.values.seconds<0&&(t.values.seconds=59,--t.values.minutes),0<=t.values.hours&&t.values.minutes<0&&(t.values.minutes=59,--t.values.hours),0<=t.values.days&&t.values.hours<0&&(t.values.hours=59,--t.values.days),// Update DOM values
// Days
t.checkValue(t.values.days,s,o),// Hours
t.checkValue(t.values.hours,i,n),// Minutes
t.checkValue(t.values.minutes,u,a),// Seconds
t.checkValue(t.values.seconds,c,l),--t.total_seconds):(clearInterval(t.countdown_interval),document.querySelector(".countdown").classList.remove("active"),document.querySelector(".countdown").classList.add("expired"),document.querySelector(".countdown__expired").classList.add("active"))},1e3)},animateFigure:function e(t,s){var o=this,i=t.querySelector(".top"),n=t.querySelector(".bottom"),u=t.querySelector(".top-back"),a=t.querySelector(".bottom-back");// Before we begin, change the back value
u.querySelector("span").innerHTML=s,// Also change the back bottom value
a.querySelector("span").innerHTML=s,// Animate
i.classList.add("flipTop"),setTimeout(function(){i.classList.remove("flipTop"),i.innerHTML=s},750),u.classList.add("flipTopBack"),setTimeout(function(){u.classList.remove("flipTopBack"),n.innerHTML=s},750)},checkValue:function e(t,s,o){var i=t.toString().charAt(0),n=t.toString().charAt(1),u=s.querySelector(".top").innerHTML,a=o.querySelector(".top").innerHTML;10<=t?(
// Animate only if the figure has changed
u!==i&&this.animateFigure(s,i),a!==n&&this.animateFigure(o,n)):(
// If we are under 10, replace first figure with 0
"0"!==u&&this.animateFigure(s,0),a!==i&&this.animateFigure(o,i))}};// Initialize countdown
Countdown.init();
